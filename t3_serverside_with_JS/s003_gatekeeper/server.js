...
function gateKeeper(req, res, next) {
  ...
  const headerInfo = queryString.parse(req.get('x-username-and-password'));
  const user = headerInfo.user || null;
  const pass = headerInfo.pass || null;
  
  // here we check to see if user exist by checking in USERS   
  req.user = USERS.find(
    (username, index) => username.userName === user && username.password === pass
  );
